<template>
  <div>
    <div style="display:flex; gap:8px; margin-bottom:8px;">
      <button class="btn-primary" @click="$emit('new')">Nueva ruta (Operación)</button>
      <button class="btn-ghost" @click="$emit('refresh')">Refrescar</button>
    </div>

    <ul style="list-style:none; padding-left:0;">
      <li v-for="tpl in opTemplates" :key="tpl.id" style="display:flex; align-items:center; gap:8px; padding:6px 0; border-bottom:1px solid #eee;">
        <span :style="{background:tpl.color||'#1565C0', width:'10px', height:'10px', borderRadius:'50%'}"></span>
        <b>{{ tpl.name }}</b> · v{{ tpl.current_version }} · {{ tpl.active?'activa':'archivada' }}
        <div style="margin-left:auto; display:flex; gap:6px;">
          <button class="btn-mini" @click="$emit('edit', tpl, regiones[0] || '')">Editar (v+1)</button>
...
<script setup>
defineProps({
  opTemplates: { type: Array, required: true },
